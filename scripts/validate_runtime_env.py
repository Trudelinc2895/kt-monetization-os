...
def validate_runtime_env(
    ...
    *,
    ...
    if target_env == "development":
        if stripe_secret.startswith("stripe_live_"):
            errors.append("Development refuses live Stripe secret keys")
        if stripe_public.startswith("stripe_public_live_"):
    ...
    if not stripe_secret and not stripe_secret_ref:
        errors.append("Production requires STRIPE_SECRET_KEY=stripe_live_...")
    elif not stripe_secret_ref and not allow_placeholders and not stripe_secret.startswith("stripe_live_"):
        errors.append("Production requires STRIPE_SECRET_KEY=stripe_live_...")

    if not stripe_public:
        errors.append("Production requires STRIPE_PUBLIC_KEY=stripe_public_live_...")
    elif not allow_placeholders and not stripe_public.startswith("stripe_public_live_"):
        errors.append("Production requires STRIPE_PUBLIC_KEY=stripe_public_live_...")

    if not stripe_webhook and not stripe_webhook_ref:
        errors.append("Production requires STRIPE_WEBHOOK_SECRET=stripe_webhook_...")
    elif not stripe_webhook_ref and not allow_placeholders and not stripe_webhook.startswith("stripe_webhook_"):
        errors.append("Production requires STRIPE_WEBHOOK_SECRET=stripe_webhook_...")
